...
class DCatAPI:
    ...
    def register_dataset(self, provenance_id: str, name: str, description: str,
                         variables: List[Dict[str, str]], url: str, filetype: str, start_time: str,
                         end_time: str, spatial_coverage: str):
        ...
        resp = requests.post(f"{DATA_CATALOG_API}/datasets/register_datasets",
        ...
        # register variables
        resp = requests.post(f"{DATA_CATALOG_API}/datasets/register_variables",
                             headers={
        ...
        resources = [{
            ...
            "metadata": {
                "spatial_coverage": {
                    "type": "BoundingBox",
                    "value": {
                        "xmin": get_boundingbox_country_output[2],
                        "ymin": get_boundingbox_country_output[0],
                        "xmax": get_boundingbox_country_output[3],
                        "ymax": get_boundingbox_country_output[1]
                    }
                },
                "temporal_coverage": {
        ...
        resp = requests.post(f"{DATA_CATALOG_API}/datasets/register_resources",
                             headers=request_headers,
                             json=resource_defs)
    ...
    def register_variables(self, variable_forms: List[StandardVariableForm]) -> List[str]:
        ...
        request_headers = {'Content-Type': "application/json", 'X-Api-Key': self.get_api_key()}
        resp = requests.post(f"{DATA_CATALOG_API}/knowledge_graph/register_standard_variables",
                             headers=request_headers,
    ...
    def find_dataset_by_id(self, provenance_id: str, dataset_id: str):
        request_headers = {'Content-Type': "application/json", 'X-Api-Key': self.get_api_key()}
        resp = requests.post(f"{DATA_CATALOG_API}/datasets/find",
                             headers=request_headers,
                             json={
    ...
    def update_layout(self, provenance_id: str, dataset_id: str, layout: dict):
        request_headers = {'Content-Type': "application/json", 'X-Api-Key': self.get_api_key()}
        resp = requests.post(f"{DATA_CATALOG_API}/datasets/update_dataset",
                             headers=request_headers,
                             json={
    ...
    def standard_variable_search(self, standard_name_uri: str) -> List[str]:
        ...
        resp = requests.post(f"{DATA_CATALOG_API}/knowledge_graph/find_standard_variables",
                             headers=request_headers,
                             json=search_query)
        parsed_response = DCatAPI.handle_api_response(resp)

        DCatAPI.logger.debug("search for variable: `%s`. Result: `%s`", standard_name_uri,
                             parsed_response)
