...
class Raster:
    ...
    def from_geotiff(infile: str) -> 'Raster':
        ...
        data = ds.ReadAsArray()
        nodata = set(
            ds.GetRasterBand(i).GetNoDataValue()
            for i in range(1, data.shape[0] + 1 if len(data.shape) > 2 else 2))
    ...
    def crop(self,
             ...
             vector_file: Union[Path, str] = None,
             use_vector_bounds: bool = True,
             x_res: float = None,
             y_res: float = None,
...
if __name__ == '__main__':
    raster = Raster.from_netcdf4(
        "/data/mint/gpm/3B-HHR-E.MS.MRG.3IMERG.20140101-S000000-E002959.0000.V06B.HDF5.nc4",
        "HQprecipitation")
