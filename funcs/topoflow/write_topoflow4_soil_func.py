...
class Topoflow4SoilWriteFunc(IFunc):
    ...
    def __init__(self, input_dir: str, output_dir: Union[str, Path], layer: str, DEM_bounds: str,
                 DEM_xres_arcsecs: str, DEM_yres_arcsecs: str):
        self.DEM = {
...
def read_soil_grid_files(input_dir, DEM_info: dict, res_str='1km', layer=1):
    ...
    for fpath in Path(input_dir).iterdir():
        if fpath.name.find(f"_M_sl{layer_str}_{res_str}") != -1 and (fpath.name.endswith(".tiff") or
                                                                     fpath.name.endswith(".TIF")):
