...
class Resource(Model):
    ...
    class Meta:
        database = SqliteDatabase(
            os.path.join(DATA_CATALOG_DOWNLOAD_DIR, "dcat_read_func.db"), timeout=10
        )
...
class ResourceManager:
    ...
    def __init__(self):
        ...
        assert (
            self.max_capacity >= self.max_clear_size
        ), "max_capacity cannot be less than max_clear_size"
        ...
        with self.db.atomic("EXCLUSIVE"):
            ...
            if "current_size" not in self.kv:
                self.kv["current_size"] = sum(
                    f.stat().st_size for f in Path(DATA_CATALOG_DOWNLOAD_DIR).rglob("*")
                )
            else:
                for resource in Resource.select():
                    ...
                    path = Path(
                        os.path.join(
                            DATA_CATALOG_DOWNLOAD_DIR,
                            str(resource.resource_id) + ".dat",
                        )
                    )
                    if not path.exists():
                        path = Path(
                            os.path.join(
                                DATA_CATALOG_DOWNLOAD_DIR, str(resource.resource_id)
                            )
                        )
                        if not path.exists():
    ...
    def download(
        ...
        is_compressed = (
            resource_metadata["resource_type"] in self.compressed_resource_types
        )
        if is_compressed:
        ...
        with self.db.atomic("EXCLUSIVE"):
            try:
                ...
                resource = (
                    Resource.select().where(Resource.resource_id == resource_id).get()
                )
                ...
                resource = Resource.create(
                    resource_id=resource_id, ref_count=1, is_downloading=True, size=0
                )
            ...
            if download:
                ...
                try:
                    resource.size = int(
                        requests.head(resource_metadata["resource_data_url"]).headers[
                            "Content-Length"
                        ]
                    )
                    required_size += resource.size
                ...
                if self.max_capacity - self.kv["current_size"] < required_size:
                    ...
                    assert (
                        self.max_capacity - self.kv["current_size"] >= required_size
        ...
        if download:
            if resource.ref_count > 1:
                ...
                DcatReadFunc.logger.debug(
                    f"Waiting for some other process/thread to free resource {resource_id} ..."
                )
            ...
            if is_compressed:
                ...
                subprocess.check_call(
                    f"wget -q \"{resource_metadata['resource_data_url']}\" -O {temp_path}",
                    shell=True,
                    close_fds=False,
                )
                ...
                required_size = -resource.size
                required_size += (
                    sum(f.stat().st_size for f in Path(path).rglob("*"))
                    + Path(path).stat().st_size
                )
                Path(temp_path).unlink()
            else:
                subprocess.check_call(
                    f"wget -q \"{resource_metadata['resource_data_url']}\" -O {path}",
                    shell=True,
                    close_fds=False,
                )
                required_size = 0
            ...
            DcatReadFunc.logger.debug(
                f"Skipping resource {resource_id}, found in cache"
            )
            if resource.is_downloading:
                # block until some other process is done downloading the resource
                DcatReadFunc.logger.debug(
                    f"Waiting for other process/thread to finish downloading resource {resource_id} ..."
                )
    ...
    def clear(self) -> int:
        size = 0
        for resource in (
            Resource.select().where(Resource.ref_count == 0).order_by(Resource.id)
        ):
            DcatReadFunc.logger.debug(f"Clearing resource {resource.resource_id}")
            path = Path(
                os.path.join(
                    DATA_CATALOG_DOWNLOAD_DIR, str(resource.resource_id) + ".dat"
                )
            )
            if path.exists():
                ...
                path = Path(
                    os.path.join(DATA_CATALOG_DOWNLOAD_DIR, str(resource.resource_id))
                )
                size += (
                    sum(f.stat().st_size for f in path.rglob("*")) + path.stat().st_size
                )
                shutil.rmtree(str(path))
            resource.delete_instance()
    ...
    def uncompress(self, resource_type: str, path: Union[Path, str]):
        subprocess.check_call(
            f"unzip {path + resource_type} -d {path}", shell=True, close_fds=False
        )
    ...
    def path(
        self, resource_id: str, path: Union[Path, str], is_compressed: bool
    ) -> str:
...
class DcatReadFunc(IFunc):
    ...
    outputs = {
        ...
        "data_path": ArgType.ListString(optional=True),
    }
    example = {
    ...
    def __init__(
        ...
        assert ("resource_repr" in dataset["metadata"]) or (
            "dataset_repr" in dataset["metadata"]
        ), "Dataset is missing both 'resource_repr' and 'dataset_repr'"
        assert not (
            ("resource_repr" in dataset["metadata"])
            and ("dataset_repr" in dataset["metadata"])
        ), "Dataset has both 'resource_repr' and 'dataset_repr'"

        resources = DCatAPI.get_instance().find_resources_by_dataset_id(
        ...
        if "resource_repr" in dataset["metadata"]:
            ...
            for resource in resources:
                self.resources[resource["resource_id"]] = {
                    key: resource[key] for key in {"resource_data_url", "resource_type"}
                }
            self.repr_type = "resource_repr"
        else:
            # TODO: fix me!!
            assert len(resources) == 1
            self.resources[resources[0]["resource_id"]] = {
    ...
    def exec(self) -> dict:
        ...
        if self.lazy_load_enabled:
                ...
                for resource_id, resource_metadata in self.resources.items():
                    dataset.add(
                        LazyLoadBackend(
                            ...
                            self.resource_manager.unlink,
                            partial(ShardedClassID, dataset.count),
                        )
                    )
                return {"data": dataset}
        else:
            ...
            if self.repr_type == "dataset_repr":
                resource_id, resource_metadata = list(self.resources.items())[0]
                resource_file = self.resource_manager.download(
                    resource_id, resource_metadata, self.should_redownload
                )
                return {
                ...
                for resource_id, resource_metadata in self.resources.items():
                    ...
                    dataset.add(
                        backend.from_drepr(
                            self.drepr, resource_file, dataset.inject_class_id
                        )
                    )
